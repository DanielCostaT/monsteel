...
// AndroidJava resources need to be #if'd in order to allow AoT compilation on iOS
    ...
    /// </summary>
    public QCARUnity.InitError Start()
    {
        int errorCode = InitQCAR();
        if (errorCode >= 0)
            InitializeSurface();
    ...
    private int InitQCAR()
    {
        int errorcode = -1;
#if UNITY_ANDROID
        LoadNativeLibrariesFromJava();
        if (mQcarInitializer != null)
            errorcode = mQcarInitializer.CallStatic<int>("initQCAR", mCurrentActivity);
#endif
        return errorcode;
    }
    ...
    private void CheckOrientation()
    {
        ...
        if (mFramesSinceLastOrientationReset < NUM_FRAMES_TO_QUERY_ORIENTATION)
        {
            ...
            SurfaceUtilities.SetSurfaceOrientation((ScreenOrientation)correctScreenOrientation);
        }
    }
